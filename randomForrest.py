...
randomForestClassifier.fit(dataTrain, targetTrain)
end = time.time()
print("Timp de antrenare pe 80% din setul de date subesantionat: ", end - start)

start = time.time()
targetUnderSamplePrediction = randomForestClassifier.predict_proba(dataTest)[:, 1] > 0.75
end = time.time()
...
start = time.time()
randomForestClassifier.fit(dataOriginalTrain, targetOriginalTrain)
end = time.time()
print("Timp de antrenare pe 80% din date: ", end - start)

start = time.time()
targetOriginalPrediction = randomForestClassifier.predict_proba(dataOriginalTest)[:,1] > 0.25
end = time.time()
